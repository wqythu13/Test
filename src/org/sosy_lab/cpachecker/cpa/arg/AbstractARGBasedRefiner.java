...
public abstract class AbstractARGBasedRefiner implements Refiner {
  ...
  public final boolean performRefinement(ReachedSet pReached) throws CPAException, InterruptedException {
    ...
    boolean isOurBreak = RefineHelper.getInstance().isRetFromMiddle();
    while(!counterexample.isSpurious() && isOurBreak) {
      //restart refinement
      RefineHelper.getInstance().setRetFromMiddle(false);
//      ARGState nTarState = RefineHelper.getInstance().getNewFlowBackwordStateToStateMap().get(path.getFirstState());
//      ARGReachedSet nReachedSet = RefineHelper.getInstance().getNewFlowStateToReachedSetMap().get(nTarState);
//      nTarState = (ARGState) nTarState.getParents().toArray()[0];
//      nTarState.setIsTar();
//      final @Nullable ARGPath npath = computePath(nTarState, nReachedSet);
//      npath.getLastState().setIsTar();
//      counterexample = performRefinement(nReachedSet, npath);
      final @Nullable ARGPath npath = computePath(lastElement, reached);
      counterexample = performRefinement(reached, npath);
    }
