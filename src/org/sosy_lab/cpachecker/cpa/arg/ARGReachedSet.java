...
public class ARGReachedSet {
  ...
  private UnmodifiableReachedSet mUnmodifiableReached;
  ...
  public void replaceReachedState(ARGState state1, ARGState state2) {
    checkNotNull(state1);
    checkNotNull(state2);
    List<AbstractState> afterStates = new ArrayList<>();
    List<Precision> afterPrecisions = new ArrayList<>();
    List<AbstractState> tmpList = new ArrayList<>();
    Iterator<AbstractState> iterator = mReached.iterator();
    ...
    boolean find = false;
    while(iterator.hasNext()) {
      AbstractState curState = iterator.next();
      if(find) {
        afterStates.add(curState);
        afterPrecisions.add(mReached.getPrecision(curState));
        mReached.remove(curState);
      }
      else if(curState.hashCode() == state1.hashCode() && curState == state1) {
        find = true;
        mReached.remove(curState);
      }
    }

    mReached.add(state2, mReached.getPrecision(mReached.getFirstState()));
    for(int i = 0; i < afterStates.size(); i++) {
      mReached.add(afterStates.get(i), afterPrecisions.get(i));
    }
    mUnmodifiableReached = new UnmodifiableReachedSetWrapper(mReached);
  }
